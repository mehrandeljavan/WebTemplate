...
public class EmployeeDBAccess
{
	...
	public Int64 Insert(Employee employee)
	{
		SqlParameter[] parametersEmployee = new SqlParameter[]
		{
			...
			new SqlParameter("@Password", (employee.Password != null) ? employee.Password : (object)DBNull.Value),
			new SqlParameter("@PersonalCode", (employee.PersonalCode != null) ? employee.PersonalCode : (object)DBNull.Value),
			new SqlParameter("@LastPassChangeDate", (employee.LastPassChangeDate != null) ? employee.LastPassChangeDate : (object)DBNull.Value),
			...
			new SqlParameter("@IsActiveUser", (employee.IsActiveUser != null) ? employee.IsActiveUser : (object)DBNull.Value)
		};
	...
	public List<Employee> SearchLike(Employee employee)
	{
		...
		SqlParameter[] parametersEmployee = new SqlParameter[]
		{
			...
			new SqlParameter("@LastPassChangeDate", employee.LastPassChangeDate),
			new SqlParameter("@FirstName", employee.FirstName),
			new SqlParameter("@LastName", employee.LastName),
			...
			new SqlParameter("@IsActiveEmployee", employee.IsActiveEmployee),
			new SqlParameter("@IsActiveUser", employee.IsActiveUser),
		};
		using (DataTable table = SqlDBHelper.ExecuteParamerizedSelectCommand("Employee_SearchLike", CommandType.StoredProcedure, parametersEmployee))
		{
			if (table.Rows.Count > 0)
			{
				foreach (DataRow row in table.Rows)
				{
					...
					tmpEmployee.Password = row["Password"].ToString();
					tmpEmployee.PersonalCode = row["PersonalCode"].ToString();
					tmpEmployee.LastPassChangeDate = row["LastPassChangeDate"].ToString();
					...
					tmpEmployee.IsActiveUser = (row["IsActiveUser"] != DBNull.Value) ? Convert.ToBoolean((row["IsActiveUser"].ToString() == "1" || row["IsActiveUser"].ToString().ToLower() == "true" ) ? true : false) : false ;
